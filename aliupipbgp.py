...
def fetch_telecom_ips():
    print(f"{get_beijing_time()} - 开始提取多线IP...")
    ...
    for row in soup.find_all("tr"):
        ...
        if len(cols) >= 2 and cols[0].get_text(strip=True) == "多线":
    ...
    print(f"{get_beijing_time()} - 提取完成，共 {len(ips)} 个多线 IP。已保存到 {OUTPUT_FILE}")
